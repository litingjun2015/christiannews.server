  ...
  async.eachSeries(list, function (item, next) {
    ...
    db.query('SELECT * FROM `class_list` WHERE `name`=? LIMIT 1', [item.name], function (err, data) {
      ...
      if (Array.isArray(data) && data.length >= 1) {
        ...
        db.query('UPDATE `class_list` SET `name`=?, `url`=? WHERE `id`=?', [item.name, item.url, data[0].id], next);
        item.id = data[0].id;
        ...
        // 分类不存在，添加
        db.query('INSERT INTO `class_list`(`id`, `category_name`, `name`, `url`) VALUES (?, ?, ?, ?)', [item.id, item.category_name, item.name, item.url], next);
        newclassList.push(item);
      }
    });
